...
import { Paper, Stack, Slider, Text, Box, Group, Tooltip, ActionIcon, Collapse } from "@mantine/core";
import { Sliders, Eye, Maximize2, Copy, Ticket, ChevronUp, ChevronDown } from "lucide-react";
...
export default function StyleControllerPanel() {
  ...
  return (
    <Paper
      ...
      radius="md"
      className="fixed bottom-6 left-6 z-50 bg-zinc-900/90 border-zinc-800 backdrop-blur-md shadow-2xl select-none transition-all duration-200 ease-in-out"
      style={{ width: 260 }}
    >
      <Stack gap="xs">
        
        {/* ACTION PANEL TITLE / MINIMIZE HEADER TRIGGER */}
        <Group 
        ...
        >
          ...
          <ActionIcon variant="transparent" size="xs" color="zinc.4" style={{ pointerEvents: 'none' }}>
            {isMinimized ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
          </ActionIcon>
        </Group>

        {/* CSS-DRIVEN ACCORDION CONTAINER OVERIDING BINDING ISSUES */}
        <div 
          className={`transition-all duration-300 ease-in-out overflow-hidden ${
            isMinimized ? "max-h-0 opacity-0 pointer-events-none" : "max-h-[400px] opacity-100 pt-2"
          }`}
        >
          <Stack gap="xs">
                  ...
                  <Group justify="space-between">
                    ...
                    <Group gap={4}>
                      ...
                      <Tooltip label={copiedCode ? "Copied!" : "Copy Code"} position="top" withArrow>
                        <ActionIcon size="xs" variant="subtle" color="teal" onClick={(e) => { e.stopPropagation(); handleCopyText({text: activeRoom.roomCode, type: 'code'}) }}>
                          <Copy size={10} />
                        </ActionIcon>
                      </Tooltip>
                    </Group>
                    ...
                    <Group gap={4}>
                      ...
                      {activeRoom.passwordPlain && (
                        <Tooltip label={copiedPass ? "Copied!" : "Copy Password"} position="top" withArrow>
                          <ActionIcon size="xs" variant="subtle" color="teal" onClick={(e) => { e.stopPropagation(); handleCopyText({text: activeRoom.passwordPlain, type: 'pass'}) }}>
                            <Copy size={10} />
                          </ActionIcon>
                        </Tooltip>
                      )}
